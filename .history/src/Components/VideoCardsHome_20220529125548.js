					...
					<Link className='link' to='/videoPlayer'>
						< div className='single-card' key={index} >
							...
							<div className="video-description">
								<p className='title' onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${video.id}` }}>Title: {video.author.thumbnails.title.charAt(0).toUpperCase()}{video.author.thumbnails.title.substring(1).toLowerCase()}</p>
								<a className='channel-id' href={`https://www.youtube.com/channel/${video.channelID}`} target='_blabk'>Channel: {video.author.name.toLowerCase().charAt(0).toUpperCase()}{video.video.author.name.substring(1).toLowerCase()}</a>
