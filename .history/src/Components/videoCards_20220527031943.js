...
const VideoCards = ({ videoId, videoLink, videoStat }) => {
	...
	function viewsNum(num) {
		if (num < 1000) return num;
			...
			let nums = num / 1000000;
			return nums + ' M';
		}

		else if ((num > 1000000000) && (num < 1000000000000)) {
			let nums = nums / 1000000000;
			return nums + ' B';
		}
					...
					< div key={index} >
						...
						<div className='video-description'>
							...
							<div className="meta">
								...
								<p onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${videoId[index].id.videoId}` }}>Views: {video.snippet.channelTitle}</p>
								<p onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${videoId[index].id.videoId}` }}>By: {video.snippet.channelTitle}</p>
								<p onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${videoId[index].id.videoId}` }}>By: {video.snippet.channelTitle}</p>
