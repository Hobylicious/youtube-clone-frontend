				...
				{videoLink.data.items.map((video, index) => (
					...
					< div className='single-card' key={index} >
						...
						<div className="video-description">
							<p onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${videoId[index].id.videoId}` }}>Title: {video.snippet.title.charAt(0).toUpperCase()}{video.snippet.title.substring(1).toLowerCase()}</>
