...
const VideoCards = ({ videoId, videoLink, videoStat }) => {
	...
	// This return all clickable thumbnails cards.
		...
		if (videoStat.data.items[0] === undefined) {
			console.log("No result found")
		}
		...
		console.log(viewsCountFunc(videoStat.data.items[0].statistics.viewCount) )
				...
				{videoLink.data.items.map((video, index) => (
					...
					< div className='single-card' key={index} >
						...
						<div className="video-description">
							...
							<p className='views'>Views: {viewsCountFunc(videoStat.data.items[0].statistics.viewCount).toFixed(1)}</p>
						</div>
					</div>
