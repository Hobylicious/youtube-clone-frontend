...
const VideoCards = ({ videoId, videoLink, videoStat }) => {
	...
	if ((videoLink.length > 1) && (videoLink.data.items[0].snippet.title.toLowerCase() === 'undefined')) {
				...
				{videoLink.data.items.map((video, index) => (
					...
					< div className='single-card' key={index} >
						...
						<div className="video-description">
							<h5 onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${videoId[index].id.videoId}` }}>Title: {video.snippet.title.charAt(0).toUpperCase()}{video.snippet.title.substring(1).toLowerCase()}</h5>
							...
							<p className='channel-id'>Views: {viewsNum(videostatics.items[0].statistics.viewCount)}</p>
						</div>
					</div>
