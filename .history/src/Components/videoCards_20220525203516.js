...
const VideoCards = ({ videoLink }) => {
	...
	if (videoLink.length < 1) {
		...
		const iframe = document.getElementsByTagName('iframe').src = 
		console.log(iframe.setAttribute('src', 'https://www.youtube.com/channel/UCcabW7890RKJzL968QWEykA'))
		...
		return (
			<div className="videosContainer">
				{videoLink.data.items.map((video, index) => (
					...
					<div key={index}>
						<img className='single-card' src={video.snippet.thumbnails.default.url} alt='images' width="120" height="90"  />
					</div>
