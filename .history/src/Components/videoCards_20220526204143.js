	...
	// This return all clickable thumbnails cards.
		...
		return (
			...
			<div>
				...
				: (
				<ul className="items">
					{videoLink.data.items.map((item, index) => (
						<li className="item" key={index}>
							<div>
								<b><a href={video.snippet.title}>{video.snippet.title}</a></b>
								<p>{item.description}</p>
							</div>
							<ul className="meta">
								<li>By: <a href=`https://www.youtube.com/channel/${video.snippet.channelId}`>{video.snippet.title}</a></li>
