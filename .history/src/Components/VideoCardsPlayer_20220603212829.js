						...
						<Link className='link' to={`/videoPlayer/${video.id}`}>
							< div key={video.id} className='single-card-player'  >
								...
								<div className="video-description">
									<p className='title playing-video-uploadedAt' onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${video.id}` }}>Title: {video.title.charAt(0).toUpperCase()}{video.title.charAt(1).toUpperCase()}</p>
