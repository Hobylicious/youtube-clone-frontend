						...
						<Link className='link' to={`/videoPlayer/${video.id}`}>
							< div key={video.id} className='single-card-player single-card'  >
								...
								<div className="video-description">
									<p className='playing-video-uploadedAt' onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${video.id}` }}>Title: {video.title}</p>
