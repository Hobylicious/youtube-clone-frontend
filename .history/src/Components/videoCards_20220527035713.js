				...
				{videoLink.data.items.map((video, index) => (
					...
					< div key={index} >
						...
						<div className='video-description'>
							...
							<div className="meta">
								<p onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${videoId.items[index].id.videoId}` }}>By: {video.snippet.channelTitle}</p>
								<p onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${videoId.items[index].id.videoId}` }}>Views: {viewsNum(videoStat.items[index].statistics.viewCount)}</p>
								<p onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${videoId.items[index].id.videoId}` }}>Favorites: {viewsNum(videoStat.items[index].statistics.favoriteCount)}</p>
								<p onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${videoId.items[index].id.videoId}` }}>Likes: {viewsNum(videoStat.items[index].statistics.likeCount)}</p>
								<p onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${videoId.items[index].id.videoId}` }}>Uploaded at: {videoStat.items[index].snippet.publishedAt.subString(5, 7)}/{videoStat.items[index].snippet.publishedAt.subString(8, 10)}/{videoStat.items[index].snippet.publishedAt.subString(0, 4)} {videoStat.items[index].snippet.publishedAt.subString(11, 13)}:{videoStat.items[index].snippet.publishedAt.subString(14, 16)}</p>
