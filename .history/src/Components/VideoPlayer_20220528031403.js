        ...
        await axios.get(urlVideoId)
            .then((data) => {
                ...
                if (data.data) {
                    ...
                    let urlVideoStat = `${apiVideoStat.api}${data.data.items.id.videoId}&key=${apiVideoId.key}&part=snippet,statistics&fields=items(id,snippet,statistics)`

                     axios.get(urlVideoStat)
                        .then((result) => {
                             console.log(result)
                            
                            // arrayViews(result.data.items[0].statistics.viewCount);
                            // arrayLikes(result.data.items[0].statistics.likeCount)
                            setVideoStat(result)
                        })
                    })
                }
    ...
    return (
        <div className='video-player-body'>
            ...
            <div className='iframe-and-videoCards-container' >
                ...
                {/* Display video cards */}
                <VideoCards videoId={videoId} videos={videos} videoStat={videoStat}  />
