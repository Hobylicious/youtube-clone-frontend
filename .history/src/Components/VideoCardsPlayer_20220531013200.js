...
const VideoCards = ({ videos, suggestedVodeos }) => {
	...
	capitalizeFirstLetter('Add to playlist')
						...
						<Link className='link' to={`/videoPlayer/${video.id}`}>
							< div className='single-card-player' key={index} >
								...
								<div className="video-description">
									<p className='title' onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${video.id}` }}>Title: {capitalizeFirstLetter(video.title)}</p>
									<div className='logo-cannel-views-posted-duration' >
										<div className='channel-info' >
											<div className='logo-channelName'>
												...
												<a className='channel-id' href={`https://www.youtube.com/channel/${capitalizeFirstLetter(video.author.channelID)}`} target='_blank' rel='noreferrer'>{video.author.name}</a>
											</div>
