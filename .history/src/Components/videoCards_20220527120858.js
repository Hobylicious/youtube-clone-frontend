...
const VideoCards = ({ videoId, videoLink, videoStat }) => {
	...
	// This return all clickable thumbnails cards.
		...
		let channelIcon = document.getElementsByClassName('ytp-title-channel');
				...
				{videoLink.data.items.map((video, index) => (
					...
					< div className='single-card' key={index} >
						...
						<div className="video-description">
							<h5>{video.snippet.title.toLowerCase()}</h5>
							<p className='channel-id' onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${videoId[index].id.videoId}` }}>By: {video.snippet.channelTitle}</p>
