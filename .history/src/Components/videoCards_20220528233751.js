					...
					<Link to='/videoPlayer'>
						< div className='single-card-home single-card-player' key={1} >
							...
							<div className="video-description-home video-description-player" key={2}>
								<p key className='title-home title-player' onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${video.id.videoId}` }}>Title: {video.snippet.title.charAt(0).toUpperCase()}{video.snippet.title.substring(1).toLowerCase()}</p>
