...
const VideoCards = ({ videoId, videoLink, videoStat }) => {
	...
	// This return all clickable thumbnails cards.
		...
		console.log(viewsNum(videostatics.items[0].statistics.videoId))

		// let channelIcon = document.getElementsByClassName('ytp-title-channel-logo');
		// console.log(channelIcon)

		if (videoLink.data.items[0].snippet.title.toLowerCase() === 'undefined') {
			setViews(0)
		}
...
setView()
						<div className="video-description">
							...
							<p className='channel-id'>Views: {viewsNum(videostatics.items[0].statistics.videoId)}</p>
						</div>
					</div>
