...
const VideoCards = ({ videos, suggestedVodeos }) => {
	...
	if (videos.length < 1) {
		...
		<>
			<div id="myModal" className="modal-player">
				<div className="modal-content-player">
					<h1>Please wait...</h1>
				</div>
			</div>
		</>
		...
		return (
			<div className="cards-player" >
				{videos.map((video, index) =>
					...
					video.type === 'video' && (
						<Link key={video.id} className='link' to={`/videoPlayer/${video.id}`}>
							< div key={index} className='single-card-player' key={index} >
