...
function VideoPlayer({ getIndex }) {
    ...
    const [likesDisLikeCount, setLikesCount] = useState(0);
    ...
    const increment = () => {
        likesCount === 0 ? setLikesCount(likesCount + 1) : setLikesCount(likesCount);
    }

    // icreament likes count by 1
    const decrement = () => {
        disLikesCount === 0 ? setDisLikesCount(dikesCount + 1) : setLikesCount(likesCount);
    }
