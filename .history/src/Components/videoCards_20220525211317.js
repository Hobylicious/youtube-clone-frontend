...
const VideoCards = ({ videoId, videoLink }) => {
	...
	if (videoLink.length < 1) {
		...
		return (
			<div className="videosContainer">function changeIframeSource(videoid) {
				...
				{videoLink.data.items.map((video, index) => (
					...
					<div key={index}>
						<img className='single-card' src={video.snippet.thumbnails.default.url} alt='images' width="120" height="90" onClick={changeIframeSource(videoId[index].id.videoId)}/>
					</div>
