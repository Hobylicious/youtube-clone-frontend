					...
					<Link className='link' to='/videoPlayer'>
						< div className='single-card' key={index} >
							...
							<div className="video-description">
								<p className='title' onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${video.id}` }}>Title: {video.thumbnail.title.charAt(0).toUpperCase()}{video.thumbnails.title.substring(1).toLowerCase()}</p>
