				...
				{videoLink.data.items.map((video, index) => (
					...
					< div className='single-card' key={index} >
						...
						<div className="video-description">
							<h5 onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${videoId[index].id.videoId}` }}>{video.snippet.title.toLowerCase()}</h5>
							<a className='channel-id' {/* onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/channel/UCo7hxTE3HeKVuu4K7N0dMYQ` }}  href= {`https://www.youtube.com/channel/${video.snippet.channelId}`}>Channel: {video.snippet.channelTitle}</a>
							...
							{/* onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/channel/UCo7hxTE3HeKVuu4K7N0dMYQ` }} */}
						</div>
					</div>
