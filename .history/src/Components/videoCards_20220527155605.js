...
console.log(videoLink)
	...
	// This return all clickable thumbnails cards.
		...
		// console.log(videoStat.data.items[0].statistics.viewCount);
				...
				{videoLink.data.items.map((video, index) => (
					...
					< div className='single-card' key={index} >
						...
						<div className="video-description">
							...
							<p className='title' onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${videoId[index].id.playlistId}` }}>Playlist: {video.snippet.title.charAt(0).toUpperCase()}{video.snippet.title.substring(1).toLowerCase()}</p>
							<p className='views'>Views: { }</p>
							https://www.youtube.com/watch?v=T69CmY6Kc5g&list=PLJbt8rLUOcEy8oiEd-jF0w4oU-s7I-c7A
						</div>
					</div>
