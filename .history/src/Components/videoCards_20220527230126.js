				...
				< div className='single-card' key={0} >
					< div className='single-card' key={0} ></div>
						...
						<div className="video-description">
							<p className='title' onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${video.id}}` }}>Title: {videos[0].title.charAt(0).toUpperCase()}{videos[0].title.substring(1).toLowerCase()}</p>
							<a className='channel-id' href={`https://www.youtube.com/channel/${videos[0].author.channelID}`}>Channel: {videos[0].author.name.toLowerCase().charAt(0).toUpperCase()}{video.author.name.substring(1).toLowerCase()}</a>
