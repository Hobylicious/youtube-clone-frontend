...
const VideoCards = ({ videoId, videoLink }) => {
	...
	else if ((videoLink.data.items[0].id.videoId) == 'undefined') {
		return (
			<div className="videosContainer">
				{videoLink.data.items.map((video, index) => (
					< div key={index} >
						<img className='single-card' src={video.snippet.thumbnails.default.url} alt='images' width="120" height="90" onClick={() => { console.log('Clicked') }} />
					</div>
				))}
