...
const VideoCards = ({ videos, suggestedVodeos }) => {
	...
	// This function Capitalizes every first char of the word.
	function titleCase(str) {
		let splitStr = str.toLowerCase().split('');
		for (let i = 0; i < splitStr.length; i++) {
			splitStr[i] = splitStr[i].charAt(0).toUpperCase() + splitStr[i].substring(1).to;
		}
				...
				{videos.map((video, index) =>
					...
					video.type === 'video' && (
						<Link className='link' to={`/videoPlayer/${video.id}`}>
							< div className='single-card-player' key={index} >
								...
								<div className="video-description">
									<p className='title' onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=${video.id}` }}>Title: {titleCase(video.title)}</p>
									<div className='logo-cannel-views-posted-duration' >
										<div className='channel-info' >
											<div className='logo-channelName'>
												...
												<a className='channel-id' href={`https://www.youtube.com/channel/${titleCase(video.author.channelID)}`} target='_blank' rel='noreferrer'>{titleCase(video.author.name)}</a>
											</div>
