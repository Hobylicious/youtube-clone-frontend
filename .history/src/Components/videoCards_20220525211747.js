...
const VideoCards = ({ videoId, videoLink }) => {
	...
	if (videoLink.length < 1) {
		...
		function changeIframeSource() {
			let url = `https://www.youtube.com/watch_popup?v=${videoId[index].id.videoId`
}`;
		...
		return (
			<div className="videosContainer">
				{videoLink.data.items.map((video, index) => (
					< div key={index} >
						<img className='single-card' src={video.snippet.thumbnails.default.url} alt='images' width="120" height="90" onClick={()=>{document.getElementsByTagName('iframe').src =}} />
					</div>
				))}
