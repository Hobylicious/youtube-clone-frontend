				...
				{videoLink.data.items.map((video, index) => (
					...
					< div className='single-card' key={index} >
						...
						<div className="video-description">
							...
							<p className='views' onClick={() => { document.getElementsByTagName('iframe')[0].src = `https://www.youtube.com/watch_popup?v=T69CmY6Kc5g&list=${videoId[index].id.playlistId}` }}>Posted at: {video.snippet.publishedAt.substring(5, 7)}/{video.snippet.publishedAt.substring(8, 10)}/{video.snippet.publishedAt.substring(0, 4)} at {video.snippet.publishedAt.substring(11, 13)}</p>
